...
const Skills = () => {
  ...
  return (
    <section id="skills" className="py-12 md:py-20 lg:py-32">
      <div className="container mx-auto px-4 sm:px-6">
        ...
        >
            <h2 className="text-3xl sm:text-4xl lg:text-5xl font-headline text-center mb-8 md:mb-12">Technical Skills</h2>
        </motion.div>
          ...
          {skills.map((category: SkillCategory, index) => (
            <motion.div key={index} variants={itemVariants}>
              <Card className="h-full shadow-md hover:shadow-xl transition-shadow duration-300">
                <CardHeader>
                  <CardTitle className="text-xl sm:text-2xl font-headline text-primary">{category.name}</CardTitle>
                </CardHeader>
                <CardContent>
                  ...
                  <div className="grid grid-cols-2 gap-4">
                    {[...Array(futureSkillsCount)].map((_, i) => (
                       <div key={i} className="flex flex-col items-center justify-center p-4 border border-dashed rounded-lg h-24">
                         <BrainCircuit className="h-6 w-6 sm:h-8 sm:w-8 text-muted-foreground/50" />
                       </div>
                    ))}
                  </div>
                </CardContent>
